...
# Calculate total energy of the grid with periodic boundary conditions
def E(grid):
    energy = -J * (np.sum(grid * np.roll(grid, shift=1, axis=0)) +
                   np.sum(grid * np.roll(grid, shift=1, axis=1)))
    return energy

# Monte Carlo step using random site selection (Metropolis algorithm)
def MC_step(grid, T):
    side = len(grid)
...
def MC_step_sistematic(grid, T):
    ...
    for i in range(side):
        for j in range(side):
            # Calculate energy change for flipping spin at (i, j)
            dE = 2 * J * grid[i,j] * (grid[(i+1) % side,j] + grid[(i-1) % side,j] + grid[i,(j+1) % side] + grid[i,(j-1) % side])
            # Metropolis criterion: accept if energy decreases or with probability exp(-dE/T)
            if dE < 0 or np.random.rand() < np.exp(-dE / T):
                dE_n += dE
                grid[i,j] *= -1
    return grid, dE_n

# Critical behavior of magnetization near Tc
def magnetization_crit(T, T_c, beta):
    return (T_c - T)**beta

# Onsager's exact solution for 2D Ising model magnetization
def magnetization_2(T, beta):
    return (1 - np.sinh(2 / T)**(-4))**beta

# Exact energy for 2D Ising model
def energy(T):
...
# Derivative of Entropy with respect to Temperature (related to Specific Heat C = T * dS/dT)
def dSdT(T, T_0, beta, A):
    T_reflected = 2 * np.sqrt(T_0**2 - 2*beta**2) - T
    return A / np.sqrt((T_0**2 - T_reflected**2)**2 + (2 * beta * T_reflected)**2) / T

n_side = 20 # Lattice size (20x20)
n_steps = 250 # Base number of Monte Carlo steps
...
n_points = 100
# Generate points concentrated around Tc using truncated normal distribution
T_dense = truncnorm.rvs((T_0 - T_center) / 0.3, (T_n - T_center) / 0.3, loc=T_center, scale=0.3, size=int(0.8 * n_points))
# Sparse points for the rest of the range
...
# Main simulation loop over temperatures
for n in range(len(T_val)):
    T = T_val[n]
    # More steps closer to T=2.2
    # Increase steps closer to Tc to handle critical slowing down
    weight = np.exp(-((T - T_center) / 0.12)**2)  # Gaussian weighting
    n_steps = int(150 + 1000 * weight)

    E_n = -2 * n_side**2 * J # Initial energy (all spins aligned)
    spins = np.ones((n_side,n_side)) # Initial configuration (cold start)
    M_samples = []
    E_samples = []
    # Monte Carlo simulation for current Temperature
    ...
    # Fluctuation-dissipation theorem for C and chi
...
M_T = np.abs(M_T) # Use absolute magnetization
...
# Calculate Entropy by integrating dS/dT
for T in T_val:
